      ...
      _export("NetworkConnect", NetworkConnect = (_dec = ccclass("NetworkConnect"), _dec2 = property({
            ...
            try {
              ...
              _this.room.onMessage("timer", message => {
                ...
                if (message < 30 && message > 0) {
                  _this.AudioController.onAudio(3);
                }

                if (message === 0) {
                  _this.AudioController.onAudio(4);
                }
              });

              _this.room.onMessage("winnerList", message => {
                console.log("WinList", message);

                _this.PayoutAnim(message);

                _this.winAnimPlus(message);
              });
              ...
              _this.room.onMessage("result", message => {
                ...
                _this.result = message.result;
              });

              _this.room.onMessage("userBet", message => {
                if (message.playerID !== _this.room.sessionId) {
                  console.log("message", message);
                  _this.UserBet = message.betType;

                  _this.createSpriteNode(message.playerID);
                } else {
                  console.log("false");
                }
              });

              _this.room.onMessage("balance", message => {
                _this.balanceUser = message.balance;
                console.log(message);
              });

              setInterval(() => {
                _this.room.send("getBalance");
              }, 500);
              ...
              _this.room.onStateChange(state => {
                ...
                _this.betDragon = state.totalBetDragon;
                _this.betTiger = state.totalBetTiger;
                _this.betTie = state.totalBetTie;
                _this.totaluserBetDragon = state.totalUserBetDragon;
                _this.totaluserBetTiger = state.totalUserBetTiger;
                _this.totaluserBetTie = state.totalUserBetTie; // console.log(this.room.state);

                _this.currentHost = state.currentHostId;
                console.log(_this.currentHost);
        ...
        createSpriteNodePay(posX, posY, spriteFrames) {
          ...
          var _loop = function _loop() {
            ...
            _this2.PayUser.addChild(spriteNode); // Giả sử winner.node là node của người chiến thắng
